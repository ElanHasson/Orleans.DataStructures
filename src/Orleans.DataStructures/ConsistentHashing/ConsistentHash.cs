...
namespace Orleans.DataStructures.ConsistentHashing
{
    class ConsistentHash<T>
    {
        SortedDictionary<int, T> circle = new();
        static int _replicate = 100;    //default _replicate count
        ...
        public  ConsistentHash(IEnumerable<T> nodes): this (nodes, _replicate)
        {
            
        }

        public  ConsistentHash(IEnumerable<T> nodes, int replicate)
        {
